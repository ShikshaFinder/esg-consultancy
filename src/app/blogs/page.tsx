      ...
      <main>
        ...
        <section className="relative pt-32 pb-20 px-6 bg-[#060e1a] overflow-hidden">
          ...
          <motion.div className="max-w-4xl mx-auto text-center relative z-10" initial="hidden" animate="show" variants={stagger}>
            <motion.span variants={fadeUp} className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#9CD5FF] block mb-4">
              Insights & Guides
            </motion.span>
          ...
          <section className="py-16 px-6 bg-[#0a1628]">
            <motion.div className="max-w-7xl mx-auto" initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.05 }} variants={stagger}>
              <motion.div variants={fadeUp} className="flex items-center gap-2 mb-8">
                <Sparkles className="w-5 h-5 text-[#9CD5FF]" />
                  ...
                  <motion.div key={post.slug} custom={i} variants={fadeUp}>
                    <Link href={`/blogs/${post.slug}`} className="group block rounded-2xl border border-white/[0.06] bg-white/[0.03] p-8 hover:border-[#7AAACE]/30 hover:bg-white/[0.05] transition-all duration-300 h-full">
                      <div className="flex items-center gap-2 mb-4">
                        <span className="text-[10px] font-bold text-[#9CD5FF] bg-[#9CD5FF]/10 border border-[#9CD5FF]/20 px-2.5 py-1 rounded-full uppercase tracking-wider">
                          Featured
                        </span>
                      ...
                      <h3 className="text-white font-bold text-xl mb-3 group-hover:text-[#9CD5FF] transition-colors duration-200 leading-snug">
                        {post.title}
                      </h3>
        ...
        <section className="py-20 px-6 bg-[#F7F8F0]">
          <motion.div className="max-w-7xl mx-auto" initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.05 }} variants={stagger}>
            ...
            <motion.div variants={fadeUp} className="text-center mb-10">
              <span className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#355872]/50 block mb-2">Browse by Topic</span>
              <h2 className="text-3xl md:text-4xl font-bold text-[#0a1628]">All Articles</h2>
            </motion.div>
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
                      selectedCategory === cat
                        ? "bg-gradient-to-r from-[#355872] to-[#7AAACE] text-white shadow-md shadow-[#355872]/20"
                        : "bg-white border border-[#355872]/10 text-[#0a1628]/60 hover:border-[#355872]/25 hover:text-[#355872]"
                    }`}
                  >
                    {iconMap[cat]}
                    {cat}
              ...
              {filteredBlogs.map((post, i) => (
                <motion.div key={post.slug} custom={i} variants={fadeUp}>
                  ...
                  >
                    {/* Category Tag */}
                    <div className="flex items-center gap-2 mb-4">
                      <span className="flex items-center gap-1 text-[10px] font-bold text-[#355872] bg-[#9CD5FF]/15 border border-[#9CD5FF]/20 px-2.5 py-1 rounded-full uppercase tracking-wider">
                        <Tag className="w-3 h-3" />
                        {post.category}
                      </span>
                      {post.featured && (
                        <span className="flex items-center gap-1 text-[10px] font-bold text-[#7AAACE] bg-[#7AAACE]/10 border border-[#7AAACE]/15 px-2 py-1 rounded-full">
                    ...
                    {/* Title */}
                    <h3 className="text-[#0a1628] font-bold text-lg mb-2 group-hover:text-[#355872] transition-colors duration-200 leading-snug">
                      {post.title}
                    </h3>

                    {/* Excerpt */}
                    <p className="text-[#0a1628]/50 text-sm leading-relaxed mb-5 line-clamp-3 flex-1">
                      {post.excerpt}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {post.tags.map((tag) => (
                        <span key={tag} className="text-[10px] text-[#355872]/60 bg-[#355872]/[0.05] px-2 py-0.5 rounded-md">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-[#355872]/[0.06]">
                      <div className="flex items-center gap-3 text-[#0a1628]/35 text-xs">
                      ...
                      <span className="flex items-center gap-1 text-[#355872] text-xs font-medium group-hover:gap-2 transition-all duration-200">
            ...
            {filteredBlogs.length === 0 && (
              <motion.div variants={fadeUp} className="text-center py-16">
                <BookOpen className="w-12 h-12 text-[#355872]/20 mx-auto mb-4" />
                <p className="text-[#0a1628]/40 text-lg font-medium">No articles in this category yet.</p>
                <p className="text-[#0a1628]/25 text-sm mt-1">Check back soon — we&apos;re always adding new content.</p>
              </motion.div>
            )}
          </motion.div>
        </section>
