...
export default function BrochurePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.open("https://wa.me/918487828752?text=Hi%2C%20I%27d%20like%20to%20download%20the%20Grow%20Bridge%20brochure.%20Name%3A%20" + encodeURIComponent(formData.name), "_blank")
  }
      ...
      <main>
        ...
        <section className="relative pt-32 pb-20 px-6 bg-[#060e1a] overflow-hidden">
          ...
          <motion.div className="max-w-4xl mx-auto text-center relative z-10" initial="hidden" animate="show" variants={stagger}>
            <motion.span variants={fadeUp} className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#9CD5FF] block mb-4">📄 Company Brochure</motion.span>
            <motion.h1 variants={fadeUp} className="text-4xl md:text-6xl font-bold text-white leading-tight mb-6">
              Download Our{" "}
              <span style={{ background: "linear-gradient(90deg,#9CD5FF,#355872)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Business Solutions</span>{" "}
              Brochure
            </motion.h1>
            ...
            <motion.div variants={fadeUp} className="flex items-center justify-center gap-6 mt-8 flex-wrap">
              {trustSignals.map((s) => (
                <div key={s.label} className="text-center">
                  <p className="text-xl font-black text-[#9CD5FF]">{s.value}</p>
          ...
          <motion.div className="max-w-5xl mx-auto" initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}>
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
              ...
              <motion.div variants={fadeUp} className="lg:col-span-3">
                <h2 className="text-2xl md:text-3xl font-bold text-[#0a1628] mb-2">Get Your Free Brochure</h2>
                <p className="text-[#0a1628]/50 text-sm mb-8">Fill in your details below to receive instant access to our comprehensive business solutions brochure.</p>
                ...
                {!submitted ? (
                  <form className="space-y-5" onSubmit={handleSubmit}>
                    <div>
                      ...
                      <div className="relative">
                        <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#0a1628]/25" />
                        <input type="text" required placeholder="Rahul Sharma"
                          value={formData.name}
                          onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                          className="w-full h-12 pl-11 pr-4 rounded-xl border border-[#355872]/10 bg-white text-[#0a1628] text-sm placeholder:text-[#0a1628]/25 focus:border-[#355872]/30 focus:ring-2 focus:ring-[#355872]/10 outline-none transition-all" />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="relative">
                          <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#0a1628]/25" />
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#0a1628]/25" />
                    ...
                    <p className="text-[#0a1628]/30 text-[11px] text-center">By submitting, you agree to receive communications from Grow Bridge. We respect your privacy.</p>
                  </form>
                ) : (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl border border-green-200 bg-green-50 p-8 text-center">
                    ...
                    <h3 className="text-[#0a1628] font-bold text-xl mb-2">Thank You, {formData.name}!</h3>
                    <p className="text-[#0a1628]/50 text-sm mb-4">Your brochure download is being prepared. Our team will also reach out to assist you with any questions.</p>
              ...
              <motion.div variants={fadeUp} className="lg:col-span-2 flex flex-col gap-5">
                <div className="rounded-2xl border border-[#355872]/10 bg-white p-6">
                  <BookOpen className="w-8 h-8 text-[#355872] mb-3" />
                  <h3 className="text-[#0a1628] font-bold text-lg mb-1">What&apos;s Inside?</h3>
                  <p className="text-[#0a1628]/50 text-sm mb-4">A complete overview of our services, success stories, pricing guidance, and step-by-step process for each service.</p>
                  <div className="space-y-2">
                    {["Service Portfolio", "Funding Roadmap", "Success Case Studies", "Process & Timelines"].map((item) => (
                      <div key={item} className="flex items-center gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5 text-[#7AAACE]" />
                        <span className="text-[#0a1628]/60 text-sm">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-[#355872]/10 bg-white p-6">
                  <Shield className="w-8 h-8 text-[#7AAACE] mb-3" />
                  <h3 className="text-[#0a1628] font-bold text-lg mb-1">Why Grow Bridge?</h3>
                  <ul className="space-y-2 mt-3">
                    {[
                      ...
                      <li key={item} className="flex items-start gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5 text-[#355872] mt-0.5 shrink-0" />
                        <span className="text-[#0a1628]/60 text-xs">{item}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="rounded-2xl border border-[#355872]/10 bg-gradient-to-br from-[#355872]/5 to-[#7AAACE]/5 p-6 flex-1">
                  <Lightbulb className="w-8 h-8 text-[#9CD5FF] mb-3" />
                  <h3 className="text-[#0a1628] font-bold text-lg mb-1">Need Help Choosing?</h3>
                  <p className="text-[#0a1628]/50 text-sm mb-3">Not sure which service is right for you? Our experts can guide you in a free 15-minute consultation call.</p>
        ...
        <section className="py-20 px-6 bg-white">
          <motion.div className="max-w-5xl mx-auto" initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="text-center mb-14">
              <span className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#7AAACE] block mb-3">Brochure Contents</span>
              <h2 className="text-3xl md:text-4xl font-bold text-[#0a1628]">Everything Covered Inside</h2>
              <p className="text-[#0a1628]/50 text-sm mt-3 max-w-xl mx-auto">Our brochure provides detailed insights into every service we offer, with real examples and clear timelines.</p>
            </motion.div>
              ...
              {brochureHighlights.map((h, i) => (
                ...
                >
                  <div className="w-11 h-11 rounded-xl bg-[#355872]/[0.08] flex items-center justify-center mb-4 group-hover:bg-gradient-to-br group-hover:from-[#355872] group-hover:to-[#7AAACE] transition-all duration-300">
                    <h.icon className="w-5 h-5 text-[#355872] group-hover:text-white transition-colors duration-300" />
                  </div>
                  <h3 className="text-[#0a1628] font-bold text-base mb-1.5">{h.title}</h3>
                  <p className="text-[#0a1628]/50 text-sm leading-relaxed">{h.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
        ...
        <section className="py-20 px-6 bg-[#060e1a]">
          <motion.div className="max-w-5xl mx-auto" initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="text-center mb-14">
              <span className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#9CD5FF] block mb-3">Full Service List</span>
              <h2 className="text-3xl md:text-4xl font-bold text-white">
                Services Covered in the{" "}
                <span style={{ background: "linear-gradient(90deg,#9CD5FF,#355872)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Brochure</span>
              </h2>
            </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              {servicesInBrochure.map((service, i) => (
                <motion.div key={service} custom={i} variants={fadeUp} className="flex items-center gap-2.5 rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3">
                  <CheckCircle2 className="w-4 h-4 text-[#9CD5FF] shrink-0" />
                  <span className="text-white/70 text-xs font-medium">{service}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
        ...
        <section className="py-20 px-6 bg-gradient-to-r from-[#355872] to-[#7AAACE]">
          <motion.div className="max-w-3xl mx-auto text-center" initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}>
            <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-white mb-4">Ready to Transform Your Business?</motion.h2>
