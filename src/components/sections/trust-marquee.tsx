  ...
  return (
    <div className="relative py-3 bg-[#0a1628] border-y border-white/[0.06] overflow-hidden">
      ...
      <Marquee pauseOnHover className="[--duration:30s] [--gap:2rem]">
        {ITEMS.map((item) => (
          <span key={item.text} className="flex items-center gap-2 text-xs text-white/40 whitespace-nowrap group">
            <span className="text-sm group-hover:scale-110 transition-transform duration-200">{item.icon}</span>
            <span className="group-hover:text-white/60 transition-colors duration-200">{item.text}</span>
